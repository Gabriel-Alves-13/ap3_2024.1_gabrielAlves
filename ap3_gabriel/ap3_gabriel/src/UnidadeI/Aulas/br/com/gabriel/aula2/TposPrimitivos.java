package br.com.gabriel.aula2;

public class TposPrimitivos {

    public static void main(String[] args) {
        
    //Tipos Primitivos: Crie um programa que declare variáveis de todos os tipos primitivos em Java 
    //(byte, short, int, long, float, double, boolean, char)
    //e uma variável do tipo String. Atribua valores a elas e imprima esses valores no console.
    
    
    byte codigoByte = 1;
    short codigoShort = 2;
    int codigoInt = 3;
    long codigoLong = 4;
    float codigoFloat = 5;
    double codigoDuble = 6;
    boolean codigoBoolean = true;
    char codigoChar = 'A';
    String codigoString = "Alurah";
    
    System.out.println(codigoByte);
    System.out.println(codigoShort);
    System.out.println(codigoInt);
    System.out.println(codigoLong);
    System.out.println(codigoFloat);
    System.out.println(codigoDuble);
    System.out.println(codigoBoolean);
    System.out.println(codigoChar);
    System.out.println(codigoString);

    }
}
